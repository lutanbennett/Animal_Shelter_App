const en = {
  ...
  management: {
    landing: {
      ...
      tiles: {
        ...
        recurringJobs:
          "Jobs that come round every week or month — who does them, and the record of when they were done.",
      },
    },
    errors: {
    ...
    recurringJobs: {
      ...
      subtitle:
        "Jobs that come round on a calendar, like the Monday stocktake. Each date appears on the assignee's My tasks, and stays there as overdue until it is marked done or skipped.",
      couldntLoad: "Couldn't load recurring jobs",
      newJob: "New recurring job",
      empty: "No recurring jobs yet. Add the first one with New recurring job.",
      paused: "Paused",
      pause: "Pause",
      resume: "Resume",
      edit: "Edit",
      ...
      confirmDelete: (title: string) =>
        `Delete “${title}”? This can't be undone. A job that has ever been marked done or skipped can't be deleted — pause it instead.`,
      cancel: "Cancel",
      save: "Save",
      saving: "Saving…",
      saved: "Saved.",
      ...
      waitsFor: (title: string) => `After ${title}`,
      opens: (path: string) => `Opens ${path}`,
      timesOfDay: {
        morning: "Morning",
        afternoon: "Afternoon",
        evening: "Evening",
      ...
      rule: {
        ...
        everyDay: (every: number) => (every === 1 ? "Every day" : `Every day, every ${every} weeks`),
        weekly: (every: number, days: string) =>
          every === 1 ? `Every ${days}` : every === 2 ? `Every other week on ${days}` : `Every ${every} weeks on ${days}`,
        monthlyDay: (every: number, day: number) =>
          `${every === 1 ? "Monthly" : every === 3 ? "Every 3 months" : `Every ${every} months`} on day ${day}${day > 28 ? " (the last day in shorter months)" : ""}`,
        monthlyWeekday: (every: number, ordinal: string, weekday: string) =>
          `The ${ordinal} ${weekday} of ${every === 1 ? "every month" : `every ${every} months`}`,
        from: (date: string) => `from ${date}`,
        between: (from: string, to: string) => `${from} – ${to}`,
      },
      form: {
        newHeading: "New recurring job",
        editHeading: (title: string) => `Edit “${title}”`,
        title: "What to do",
        titlePlaceholder: "e.g. Stocktake of on-hand medication",
        description: "Instructions",
        descriptionPlaceholder: "Optional. Shown under the job on My tasks.",
        timeOfDay: "Time of day",
        link: "Screen it is done on",
        linkNone: "None",
        linkOther: "Other page…",
        linkPlaceholder: "/stocktake?tab=diets",
        linkHint: "A page in this app, starting with /. The job's title on My tasks opens it.",
        linkPresets: {
          stocktake: "Stocktake — Medications",
          stocktakeDiets: "Stocktake — Diets",
        ...
        repeats: {
          ...
          monthly_weekday: "Monthly, on a weekday (e.g. first Monday)",
        },
        everyWeeks: "Every how many weeks",
        everyMonths: "Every how many months",
        everyHintWeeks: "1 = every week, 2 = fortnightly. Counted from the week the job starts in.",
        everyHintMonths: "1 = every month, 3 = quarterly. Counted from the month the job starts in.",
        weekdays: "On",
        monthDay: "Day of the month",
        weekOfMonth: "Which one",
        weekday: "Weekday",
        startsOn: "Starts",
        endsOn: "Ends",
        endsOnHint: "Optional. Leave blank to repeat until it is paused.",
        dependsOn: "Do after",
        dependsOnNone: "Nothing — it can be done any time that day",
        dependsOnHint:
          "When both fall on the same day, this one shows “waiting for …” on My tasks until the other is done or skipped.",
        ...
        preview: "Next dates",
        previewLoading: "Working out the dates…",
        previewNone: "This rule doesn't fall on any date in the next two years.",
        previewNote: "Worked out by the same rule My tasks uses. Check the first one is the date you expect.",
        ruleChangeNote:
          "Changing when it repeats starts the missed-dates count again from today, so dates only the old rule had don't show as missed.",
      },
      handOver: {
        heading: "Hand over",
        intro:
          "When someone is off, give their dates to someone else without changing the jobs; the week after goes back to normal. When someone has left, move their jobs for good.",
        from: "From",
        fromPlaceholder: "Choose a person…",
        to: "To",
        mode: "For",
        modeDates: "These dates only (off sick, on leave)",
        modePermanent: "From now on (has left or changed role)",
        startDate: "From date",
        endDate: "To date",
        note: "Reason",
        notePlaceholder: "e.g. Anna off sick",
        submit: "Hand over",
        done: (n: number) => (n === 0 ? "Nothing to hand over for those dates." : `Handed over ${n} date${n === 1 ? "" : "s"}.`),
        donePermanent: (n: number) =>
          n === 0 ? "They weren't on any jobs." : `Moved ${n} job${n === 1 ? "" : "s"}.`,
        failed: "Some couldn't be handed over:",
      },
      covered: {
        heading: "Handed to someone else",
      ...
      record: {
        ...
        intro: "The latest dates marked done or skipped, with who marked them and any note.",
        empty: "Nothing has been marked yet.",
        done: "Done",
        skipped: "Skipped",
        by: (name: string, at: string) => `by ${name}, ${at}`,
      },
      errors: {
        titleRequired: "Say what the job is.",
        titleTooLong: "Keep the title to 200 characters.",
        descriptionTooLong: "Keep the instructions to 4,000 characters.",
        timeOfDayInvalid: "Choose a time of day.",
        linkInvalid: "The screen must be a page in this app, starting with a single /.",
        repeatInvalid: "Choose how it repeats.",
        everyInvalid: "Every must be a whole number from 1 to 52.",
        startRequired: "Choose the date it starts.",
        endInvalid: "The end date isn't a date.",
        endBeforeStart: "The end date is before the start date.",
        weekdaysRequired: "Tick at least one day.",
        monthDayInvalid: "The day of the month must be 1 to 31.",
        weekdayRequired: "Choose a weekday.",
        ...
        handOverToRequired: "Choose who takes them.",
        handOverDatesRequired: "Choose the dates.",
        handOverTooLong: "Hand over at most a year at a time.",
      },
    },
    stock: {
  ...
  my: {
    ...
    recurring: {
      ...
      skip: "Skip",
      addNote: "Add a note",
      notePlaceholder: "Optional note, e.g. what was short",
      outcomeFor: (title: string) => `Mark ${title}`,
      markedDone: (title: string) => `“${title}” marked done.`,
      markedSkipped: (title: string) => `“${title}” skipped.`,
