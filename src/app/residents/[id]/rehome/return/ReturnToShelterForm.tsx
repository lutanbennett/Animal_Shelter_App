...
export function ReturnToShelterForm({
  residentId,
  status,
  carerName,
  leftOn,
  ...
  /** Start of the foster / adoption placement, i.e. the date they left. */
  leftOn: string | null;
  /**
   * Where the resident was before leaving, pre-selected in the picker;
  ...
  const leftDate = leftOn ? formatDate(leftOn, locale) : null;
  const currentLine =
    status === "Adopted"
      ? carerName
  ...
  const sinceLine = leftDate
    ? carerName
      ? status === "Adopted"
        ? r.adoptedSince(carerName, leftDate)
        : r.fosteredSince(carerName, leftDate)
      : r.awaySince(leftDate)
    : null;
    ...
    <>
      ...
      >
        <div className="rounded-lg border border-border bg-surface p-4">
          ...
          {sinceLine && <p className="text-xs text-muted">{sinceLine}</p>}
        </div>
