-- Weigh at intake.
--
-- Every animal is weighed on arrival, so the intake form takes an optional
-- weight and record_intake() writes it as the first `weight` row, dated the
-- intake date, in the same transaction as the resident and the Intake
-- placement — a failed weight insert (say, a zero reading tripping 0028's
-- check) rolls the whole intake back rather than leaving a resident with no
-- placement. Null means nothing is written, so the form can leave it blank.
--
-- New parameter, so the 0011/0023 signature is dropped and re-created
