-- Links residents to group_origins (Section 4.1: "reference table for where
-- a group of animals originated, e.g. a rescue intake batch") — this table
-- existed since 0001 but nothing ever referenced it. Staff need to group
-- residents by where they came from at intake (e.g. "these 6 dogs came from
-- the temple down the road").
--
-- record_intake gains two new params: p_group_origin_id to attach an
-- existing origin, and p_new_origin_name to create one inline in the same
-- transaction (the common case — the first of a batch creates the origin,
-- the rest just pick it from the dropdown). If both are given, the new
-- origin wins, since typing a new name is an explicit intent to create one.

alter table residents
  add column if not exists group_origin_id uuid references group_origins (id);

drop function if exists record_intake(
  text, date, text, text, text, text, text, date, numeric, text, text, text,
  text, boolean, boolean, uuid, text
);
